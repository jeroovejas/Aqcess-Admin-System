...
import { toggleAddExpense } from "@/store/Slices/ExpenseSlice";
...
const AddPaymentExpense: React.FC<any> = () => {
    ...
    const [formExpenseData, setFormExpenseData] = useState<ExpenseFormData>(initialExpenseFormData);
    ...
    const handleExpenseCancel = async () => {
        dispatch(toggleAddPayment());
        setFormExpenseData(initialExpenseFormData);
    };
    ...
    const handleExpenseSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const body = new FormData();
            body.append('amount', formExpenseData.amount === null ? '' : formExpenseData.amount.toString());
            body.append('desc', formExpenseData.desc);
            body.append('token', token);
            if (formExpenseData.file) {
                body.append('attachment', formExpenseData.file);
            }
                ...
                <div className='border-0 absolute top-0 right-0 z-999 bg-white text-black w-full md:w-3/5 lg:w-2/5 h-screen overflow-y-scroll my-scrollbar outline-none focus:outline-none px-8 py-8'>
                    <div className="flex space-x-4 mb-6">
                        <button
                            ...
                            className={`flex-1 flex justify-center items-center gap-2 py-2 px-4 rounded-lg border ${selectedType === 'payment' ? 'bg-black text-white font-semibold' : 'border-gray-300'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex justify-center items-center gap-2 py-2 px-4 rounded-lg border ${selectedType === 'expense' ? 'bg-black text-white font-semibold' : 'border-gray-300'
                                }`}
                        >
                            <SiExpensify />
                                    ...
                                    <div className="w-full">
                                        ...
                                        <div className="flex items-center justify-center w-full">
                                            <label
                                                ...
                                                className="flex flex-col items-center justify-center w-full h-50 border-gray-300 border border-[#DDDDDD] rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                                            >
                                                {formExpenseData.imagePreview ? (
                                                    <div className="relative w-full h-full">

                                                        <img src={formExpenseData.imagePreview} alt="Preview" className="w-full h-full object-cover rounded-lg" />
