...
const DatePickerOne: React.FC<any> = ({ defaultDate, onChange }) => {
  ...
  useEffect(() => {
    ...
    const picker = flatpickr(datePickerRef.current!, {
      ...
      dateFormat: "d-m-Y H:i", // Date format with time
      prevArrow:
      ...
      onChange: (selectedDates, dateStr, instance) => {
        // Invoke the onChange callback with the selected date
        instance.open();
        const formattedDate = selectedDates.length > 0 ? formatDate(selectedDates[0]) : '';
        if (onChange) {
          instance.open();
